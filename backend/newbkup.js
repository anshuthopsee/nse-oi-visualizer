...
export const getIVAndGreeks = ({ futuresPrice, timeToExpiry, riskFreeRate = 0, costOfCarry = 0 }, item) => {
  ...
  if (timeToExpiry > 0) {
    ...
    ce = iv ? _gbs("c", futuresPrice, strikePrice, timeToExpiry, riskFreeRate, costOfCarry, iv) : null;
    pe = iv ? _gbs("p", futuresPrice, strikePrice, timeToExpiry, riskFreeRate, costOfCarry, iv) : null;
  };
...
export const getOptionPrice = ({ type, futuresPrice, strike, timeToExpiry, 
  ...
  if (timeToExpiry === 0 || iv === 0 || !iv) {
    return type === "c" ? Math.max(futuresPrice - strike, 0) 
    : Math.max(strike - futuresPrice, 0);
  };

  const priceAndGreeks = _gbs(type, futuresPrice, strike, timeToExpiry, riskFreeRate, costOfCarry, iv);
...
const generatePayoffRange = ({ optionLegs, lotSize, projectedFuturesPrices, 
  targetDateTime, targetUnderlyingPrice, currentUnderlyingPrice 
}) => {
  ...
  const [fivePercentDown, fivePercentUp] = [roundedPrice * 0.95, roundedPrice * 1.05];
  const [fiftyPercentDown, fiftyPercentUp] = [roundedPrice * 0.5, roundedPrice * 1.5];
  ...
  for (let i = 0; i < sortedOptionLegs.length; i++) {
    ...
    while (underlyingPrice <= maxUnderlyingPrice) {
      let step;
      if (underlyingPrice >= fivePercentDown && underlyingPrice <= fivePercentUp) {
        step = fivePercentStep;
      } else if (underlyingPrice >= fiftyPercentDown && underlyingPrice <= fiftyPercentUp) {
...
export const getPayoffData = (builderData) => {
  ...
  const { optionLegs, underlyingPrice, targetUnderlyingPrice, 
    futuresPerExpiry, targetDateTimeISOString, lotSize 
  } = builderData;
